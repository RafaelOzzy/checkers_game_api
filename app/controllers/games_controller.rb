...
#   def create
#     @game = Game.new(status: 'player_1_turn', player1_token: SecureRandom.hex(10))
#     if @game.save
...
class GamesController < ApplicationController
  ...
  def move
    ...
    if valid_move?(piece, destination)
      ...
      puts "Invalid move for piece #{piece.id} to destination #{destination.inspect}"
      render json: { error: 'Invalid move' }, status: :unprocessable_entity
    end
  end
  ...
  def valid_move?(piece, destination)
    possible_moves = calculate_possible_moves(piece)
    puts "Possible moves for piece #{piece.id}: #{possible_moves.inspect}"
    possible_moves.include?(destination)
  end
